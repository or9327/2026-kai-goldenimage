...
list_modules() {
    ...
    for category_dir in "${SCRIPT_DIR}/modules"/*; do
        if [ -d "$category_dir" ]; then
            ...
            log_info "[$category_name] $category_title"
...
execute_module() {
    ...
    if bash "$module_path"; then
        ...
        # 실패 시 처리
        if [ "${STOP_ON_ERROR:-true}" = true ]; then
...
main() {
    ...
    if [ ${#SELECTED_MODULES[@]} -gt 0 ]; then
        for module in "${SELECTED_MODULES[@]}"; do
            module_path=$(find "${SCRIPT_DIR}/modules" -name "${module}.sh")
